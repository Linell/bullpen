...
- **Games derive independently.** A derive only replaces its own game's rows, so up to three run at once. `players` and `teams` are views that pick the newest per-game bio and team row, so the order games finish in doesn't matter. Live events are tagged `reason: "live"` and backfills and rebuilds `reason: "backfill"`; an untagged event counts as backfill. Backfills use at most two of the three derive slots and one of the two ingest slots, and run after live games, so a live game never waits behind a rebuild.
